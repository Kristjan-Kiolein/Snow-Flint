...
public class EstonianIdExtensionValidatior {
  ...
  /**
   * Validates given code as estonian id. Id extension format depends on year. Current year is used.
  ...
  /**
   * Validates given code as estonian id. Id extension format depends on year. Minimum year is 1800
   * and maximum 2199.
   * @param code string to validate.
   * @param year year at which the id extension must be valid.
   * @return if given code is estonian id extension.
   */
  public static boolean isValid(String code, int year) {

    String idRegex = getIdRegex(year);
    if (code == null || idRegex == null) {
      return false;
    }
      

    code = code.trim();
    if (code.length() != ID_EXTENSION_LENGTH) {
      return false;
    }
    if (!code.matches(idRegex)) {
      return false;
    }
    ...
    if (checkSum == 10) {
      checkSum = calculateCheckSum(code, II_RANK_WEIGHT);
    }

    if (Character.getNumericValue(code.charAt(ID_EXTENSION_LENGTH - 1)) == checkSum) {
      return true;
    }
  ...
  /**
   * Calculates checksum of id extension with given weight.
  ...
  private static int calculateCheckSum(String code, String weight) {
    ...
    for (int i = 0; i < weight.length(); i++) {
      checkSum += Character.getNumericValue(code.charAt(i))
          * Character.getNumericValue(weight.charAt(i));
    }
    return checkSum % 11;
  }

  /**
   * Gets correct regex for century + sex part of the id extension (the first digit). Minimum year
   * is 1800 and maximum 2199.
